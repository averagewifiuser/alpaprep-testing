# all routes with regards to tests and their results

from alphaprep import db
from flask import render_template, url_for, flash, redirect, request, send_file, Blueprint
...
'''
OKay so the question_shuffle stores the question after the shuffle function shuffles it

the question_shuffled is a local variable to keep the instance of the shuffled questions

You'll notice a lot of indexing [] to get some values
due to how the data was returned this was the method most comfortable for the developer
put print functions below the desired data after it has gotten to understand why it was indexed as such
'''
...
def questions():
    user = current_user 
    fxns.level_checker(user)
    
    global subject
    subject = 'none'
    return render_template('questions.html', title='Questions', s = subject)
...
def english():
    ...
    # this entire side is about loading the questions based on topics for english only
    global question1
    ...
    t = 'st' + str(random.randint(1,15)) #remember to change it to rand selection of the numbers when you update the database with more stories
    global story
...
#set a variable which will receive a post from the questions page to which question they would want to answer and that would reflect as the function 
# parameter in order to affect the database
...
@tests.route('/result', methods=['POST'])
def quiz_answers():
    ...
    if subject == 'english':
        ...
        if recommendation != "None":
            global recommendation_link
            if current_user.user_level == 1:
                recommendation_link = "<form action='/tutoriallanding' method='POST' align='right|left'><button style='background-color: #f1f1f1; min-width: 160px;'name='level' type='submit' value='Level 1'>To Tutorials</button></form>"
            elif current_user.user_level == 2:
                recommendation_link = "<form action='/tutoriallanding' method='POST' align='right|left'><button style='background-color: #f1f1f1; min-width: 160px;'name='level' type='submit' value='Level 2'>To Tutorials</button></form>"
            else:
                recommendation_link = "<form action='/tutoriallanding' method='POST' align='right|left'><button style='background-color: #f1f1f1; min-width: 160px;'name='level' type='submit' value='Level 3'>To Tutorials</button></form>"
        ...
        if sessions is 1 and current_user.is_authenticated:
            ...
            progress = User_progress(user=current_user.id, last_subject = subject, last_score = correct, last_recommendation=recommendation, recommendation_link=recommendation_link)
        ...
        return render_template('result.html', title='Your Score', correct=correct, subject = subject, q1 = questions_shuffle1, q2 = qt, a1 = answer1, a2 = answer2, b1 = answered1, b2 = answered2, c1 = bans1, c2 = bans2,
        ...
        c4=bans4, c5=bans5, c6=bans6, recommendation_link=recommendation_link, correct1=correct1, correct2=correct2, count=count,
        correct3=correct3, correct4=correct4, correct5=correct5, correct6=correct6)
        ...
        return render_template('result.html', title='Your Score', correct=correct, subject = subject, q1 = questions_shuffle1, a1 = answer1, b1 = answered1, c1 = bans1, total=total, wrongs = others_wrong, recommendation = recommendation)


'''
Every flashed messages in the results page flashes to the next avaialable page that has the flashed messages catcher in its html code
So it'll either flash on the dashboard or the tutorial page
'''
